...
const Scene = () => {
  ...
  const [letters, setLetters] = useState([
    <Letter
      ...
      position={[-1, 4, 0]}
      rotation={[
        ((Math.random() * Math.PI) / 6) * (Math.random() < 0.5 ? -1 : 1),
        ((Math.random() * -Math.PI) / 6) * (Math.random() < 0.5 ? -1 : 1),
        0,
      ]}
    />,
  ...
  return (
    <>
      ...
      <spotLight position={[1, 4, 5]} angle={0.8} intensity={1} castShadow />
      <Physics
        gravity={[0, -1, 0]}
      ...
      >
        ...
        <Suspense fallback={null}>{letters}</Suspense>
      </Physics>
    </>
  );
};
